...
fn print_column_stats(col_stats: &ColumnStatistics) {
    if let Some(tstats) = col_stats.type_statistics() {
        match tstats {
            ...
            orc_rust::statistics::TypeStatistics::Timestamp { min, max, min_utc, max_utc } => {
            ...
            orc_rust::statistics::TypeStatistics::Collection { min_children, max_children, total_children } => {
...
fn main() -> Result<()> {
    ...
    println!("File {:?} has {} columns", cli.file, metadata.column_file_statistics().len());
    println!();
    ...
    println!("File {:?} has {} stripes", cli.file, metadata.stripe_metadatas().len());
    println!();
